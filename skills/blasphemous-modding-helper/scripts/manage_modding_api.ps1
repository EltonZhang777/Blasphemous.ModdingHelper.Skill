<#
.SYNOPSIS
    Cross-platform entry point for the Python ModdingAPI lifecycle manager.
#>

$ErrorActionPreference = "Continue"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$Core = Join-Path $ScriptDir "manage_modding_api.py"
$Python = $env:PYTHON3
...
if (-not (Test-Path -LiteralPath $Core -PathType Leaf)) {
    ...
    [Console]::Error.WriteLine("worktree_state: unknown")
    [Console]::Error.WriteLine("network_state: unknown")
    [Console]::Error.WriteLine("cause: Python lifecycle manager is missing: $Core")
    [Console]::Error.WriteLine("next_step: Reinstall the skill package, then retry.")
    exit 1
}

$PythonArguments = @($Core) + @($args)
