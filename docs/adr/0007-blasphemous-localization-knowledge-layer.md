...
- Add an on-demand `localization-lookup` sub-skill reached by the main skill. It is a read-only branch and does not require the Modding profile or source-code configuration. Other branches keep the existing configuration gate.
